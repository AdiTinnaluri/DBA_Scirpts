...
$ORACLE_HOME/bin/expdp \'/ as sysdba\'  schemas=${ORACLE_SID}_MDS directory=PART_DIR dumpfile=expdp_${ORACLE_SID}_MDS_`date +%m%d%y`.dmp logfile=expdp_OBI${ORACLE_SID}_MDS_`date +%m%d%y`.log COMPRESSION=ALL
...
$ORACLE_HOME/bin/expdp \'/ as sysdba\' schemas=FBI_BIA_ODIREPO directory=PART_DIR dumpfile=expdp_${ORACLE_SID}_FBI_BIA_ODIREPO_`date +%m%d%y`.dmp logfile=expdp_${ORACLE_SID}_FBI_BIA_ODIREPO_`date +%m%d%y`.log
