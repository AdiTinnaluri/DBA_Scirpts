...
prompt  -- Setting running_processes = 0 and max_processes = 0 for all
managers
...
REM     Also set Running requests to completed, since the managers are
down
...
' || col || ' = ''Y''';
        ...
        dbms_output.put_line('Updated ' || upd_rows || ' rows of ' ||
col || ' in ' || tbl || ' to ''N''');
end;
/
