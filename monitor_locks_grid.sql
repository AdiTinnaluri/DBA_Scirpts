REM $Header: Monitor_Locks_grid.sql 1.1 2013/07/12 18:00:00 IST CHNADRA MULAKALA (Z04965) Exp  $ |
REM **********************************************************************************************
REM  SCRIPT NAME
REM     Monitor_Locks_grid.sql
REM
...
REM -------  ----------   ------------------- --------------------
REM 1.0      2011-12-02   Karthick Muthaiyan  Initial Creation
REM 1.1      2013-07-12	  Chandra Mulakala    Added fields for more details as shown in Grid
REM		 
REM **********************************************************************************************

REM WHENEVER SQLERROR EXIT sql.sqlcode;

REM accept DBA_PERFORMING_TASK  prompt 'Please ENTER DBA Name running script: '
...
BEGIN
    dbms_output.put_line('=====================================================================');
      ...
      FETCH cur_c1 INTO v_sid;
		...
		select owner ||' :: '||object_name||' ('||object_type||') ' into v_obj_info from dba_objects where object_id=s.row_wait_obj#;
		ELSE
		v_obj_info := ' ';
		END IF;
		...
		dbms_output.put_line('Object      : '|| nvl(v_obj_info, 'NONE'));	
		...
		dbms_output.put_line('Previous SQL_ID : '|| s.prev_sql_id);
...
select s.inst_id,s.status,s.sid,s.serial#,p.spid,s.last_call_et,to_char(s.logon_time,'dd/mm/yy hh24:mi:ss') logon_time,s.osuser,s.username,s.module,s.program,s.action,s.machine, s.sql_id, s.row_wait_obj#
...
cursor c_bloker_ss (v_id1 NUMBER, v_id2 NUMBER, v_sid NUMBER) is
select * from gv$lock where id1=v_id1 and id2=v_id2 and sid!=v_sid;

cursor c_bsessinfo (v_bsid NUMBER, v_binst_id NUMBER) is
select s.inst_id,s.status,s.sid,s.serial#,p.spid,s.last_call_et,to_char(s.logon_time,'dd/mm/yy hh24:mi:ss') logon_time,s.osuser,s.username,s.module,s.program,s.action,s
.machine, s.sql_id, s.row_wait_obj#
from gv$session s ,gv$process p
where s.paddr = p.addr
and s.inst_id = p.inst_id
and s.sid = v_bsid
and s.inst_id = v_binst_id;
...
v_instname VARCHAR2(10);
v_mode VARCHAR2(10);
v_request VARCHAR2(10);
v_binstname VARCHAR2(10);
v_bmode VARCHAR2(10);
v_brequest VARCHAR2(10);
v_cnt  NUMBER;
v_holdercnt NUMBER;
...
BEGIN
   ...
   FOR i in 1..v_locks_tab.COUNT LOOP
        IF i = 1 THEN
        dbms_output.put_line(chr(10));
        dbms_output.put_line('Inst STATUS   SID  SERIAL# SPID    SQL_ID        LTYPE MODE_HELD MODE_REQ  OBJECT                              Time(Sec) USERNAME  MODULE       ACTION');
        dbms_output.put_line('---- -------- ---- ----- --------- ------------- ----- --------- --------  ----------------------------------- --------- --------- ----------   ---------');
        END IF;
        FOR v_sessinfo IN c_sessinfo(v_locks_tab(i).sid ,v_locks_tab(i).inst_id) LOOP
	--dbms_output.put_line('M1::'||v_locks_tab(i).sid||'::'||v_locks_tab(i).inst_id||'::'||v_sessinfo.row_wait_obj#);
	v_holdercnt := 0;
	IF v_sessinfo.row_wait_obj# > 0 THEN
	select owner ||'::'||object_name||'('||object_type||')' into v_obj_info from dba_objects where object_id=v_sessinfo.row_wait_obj#;
	ELSE
	v_obj_info := ' ';
	END IF;	
	--dbms_output.put_line('M!::Obj Info::'||v_obj_info);
	select instance_name into v_instname from gv$instance where inst_id=v_sessinfo.inst_id;
	select decode(v_locks_tab(i).lmode, 0,'None', 1,'Null', 2,'Row Share', 3,'Row Exclu',4,'Share', 5,'Share Row Ex', 6,'Exclusive') into v_mode from dual;
	select decode(v_locks_tab(i).request, 0,'None', 1,'Null', 2,'Row Share', 3,'Row Exclu', 4,'Share', 5,'Share Row Ex', 6,'Exclusive') into v_request from dual;
	IF v_locks_tab(i).request = '0' THEN
	--dbms_output.put_line(v_cnt);
	v_cnt := v_cnt+1;
	-- dbms_output.put_line('M2');
	--dbms_output.put_line(chr(10));
        --dbms_output.put_line(' + '||rpad(v_sessinfo.inst_id,3,' ')||rpad(v_sessinfo.status,9,' ')||rpad(v_sessinfo.sid,5,' ')||rpad(v_sessinfo.serial#,6,' ')||rpad(v_sessinfo.spid,9,' ')||' '||rpad(nvl(v_sessinfo.sql_id,' '),14,' ')||rpad(v_locks_tab(i).type,6,' ')||rpad(v_mode, 10,' ') ||rpad(v_request,10,' ') ||rpad(v_obj_info,40,' ')||rpad(v_locks_tab(i).ctime,6,' ')||rpad(v_sessinfo.username,9,' ')||' '||rpad(nvl(v_sessinfo.module,' '),12,' ')||' '||rpad(nvl(v_sessinfo.action,' '),13,' '));
		FOR v_bloker_ss in c_bloker_ss (v_locks_tab(i).id1, v_locks_tab(i).id2, v_locks_tab(i).sid) LOOP
			FOR v_bsessinfo in c_bsessinfo(v_bloker_ss.sid, v_bloker_ss.inst_id) LOOP
			IF v_bsessinfo.row_wait_obj# > 0 THEN
			select owner ||'::'||object_name||'('||object_type||')' into v_bobj_info from dba_objects where object_id=v_bsessinfo.row_wait_obj#;
			ELSE
			v_bobj_info := ' ';
			END IF;
			select instance_name into v_binstname from gv$instance where inst_id=v_bsessinfo.inst_id;
			select decode(v_bloker_ss.lmode, 0,'None', 1,'Null', 2,'Row Share', 3,'Row Exclu',4,'Share', 5,'Share Row Ex', 6,'Exclusive') into v_bmode from dual;
		select decode(v_bloker_ss.request, 0,'None', 1,'Null', 2,'Row Share', 3,'Row Exclu', 4,'Share', 5,'Share Row Ex', 6,'Exclusive') into v_brequest from dual;
IF v_holdercnt = 0 THEN
dbms_output.put_line(' + '||rpad(v_sessinfo.inst_id,3,' ')||rpad(v_sessinfo.status,9,' ')||rpad(v_sessinfo.sid,5,' ')||rpad(v_sessinfo.serial#,6,' ')||rpad(v_sessinfo.spid,9,' ')||' '||rpad(nvl(v_sessinfo.sql_id,' '),14,' ')||rpad(v_locks_tab(i).type,6,' ')||rpad(v_mode, 10,' ') ||rpad(v_request,10,' ') ||rpad(v_bobj_info,40,' ')||rpad(v_locks_tab(i).ctime,6,' ')||rpad(v_sessinfo.username,9,' ')||' '||rpad(nvl(v_sessinfo.module,' '),12,' ')||' '||rpad(nvl(v_sessinfo.action,' '),13,' '));
END IF;
v_holdercnt := 1;
dbms_output.put_line('- '||rpad(v_bsessinfo.inst_id,3,' ')||rpad(v_bsessinfo.status,9,' ')||rpad(v_bsessinfo.sid,5,' ')||rpad(v_bsessinfo.serial#,6,' ')||rpad(v_bsessinfo.spid,9,' ')||' '||rpad(nvl(v_bsessinfo.sql_id,' '),14,' ')||rpad(v_locks_tab(i).type,6,' ')||rpad(v_bmode, 10,' ') ||rpad(v_brequest,10,' ') ||rpad(v_bobj_info,40,' ')||rpad(v_locks_tab(i).ctime,6,' ')||rpad(v_bsessinfo.username,9,' ')||' '||rpad(nvl(v_bsessinfo.module,' '),12,' ')||' '||rpad(nvl(v_bsessinfo.action,' '),13,' '));
			END LOOP;
		END LOOP;
	END IF;
        END LOOP;
...
dbms_output.put_line(
        ...
        'DBA Executing   : &&DBA_PERFORMING_TASK ' );
dbms_output.put_line('*******************************************************************************************************************************' || CHR(10));
