create or replace PROCEDURE          "PROC_GL_BALANCE_FACT_UPD_PPE" 
AS
  /*
  Purpose : This procedure is built to update PPE columns (X_ADDITIONS, X_RETIREMENTS, X_TRANSFERS)
  in GL Balance Table
  Version : 1.0
  Date    : 09/21/14
...
BEGIN
                ...
                LAST_MONTH_PARTITION := 'P_'||TO_CHAR(LAST_DAY(ADD_MONTHS(SYSDATE,     -2)),'YYYYMMDD');
                CURR_MONTH_PARTITION := 'P_'||TO_CHAR(LAST_DAY(ADD_MONTHS(SYSDATE,     -1)),'YYYYMMDD');
                CURR_MONTH           := EXTRACT(MONTH FROM LAST_DAY(ADD_MONTHS(SYSDATE,-1)));

        PROC_INFO_LOG ('PROC_GL_BALANCE_FACT_UPD_PPE','Processing Partition: '||CURR_MONTH_PARTITION,'Processing Partition: '||CURR_MONTH_PARTITION);
        PROC_INFO_LOG ('PROC_GL_BALANCE_FACT_UPD_PPE','Set PPE columns to zero', 'Set PPE columns to zero');
        ...
        PROC_INFO_LOG ('PROC_GL_BALANCE_FACT_UPD_PPE','Set PPE columns to zero for '||VVC_REC_COUNT||' Records','Set PPE columns to zero for '||VVC_REC_COUNT||' Records');
        --PROC_INFO_LOG ('PROC_GL_BALANCE_FACT_UPD_PPE',vvc_sqlstmt,vvc_sqlstmt);
        COMMIT;
        PROC_INFO_LOG('PROC_GL_BALANCE_FACT_UPD_PPE','Updating PPE Columns','Updating PPE Columns');
        IF (curr_month = 1) THEN
                                vvc_sqlstmt := '
                                MERGE /*+ PARALLEL(TGT 8) */ INTO W_GL_BALANCE_F PARTITION ('||CURR_MONTH_PARTITION||') TGT
                                USING
                                (
                SELECT GL_ACCOUNT_WID AS GL_ACCOUNT_WID, DB_CR_IND AS DB_CR_IND,DOC_CURR_CODE,
                SUM(X_ADDITIONS) AS X_ADDITIONS,
                SUM(X_RETIREMENTS) AS X_RETIREMENTS,
                SUM(X_TRANSFERS) AS X_TRANSFERS
                FROM
                (SELECT /*+ FULL(PREV) PARALLEL(F, 8) */ F.GL_ACCOUNT_WID, DB_CR_IND,DOC_CURR_CODE,
                ...
                AND REPLACE(TGT.ACCT_CURR_CODE, 'NULL', TGT.LOC_CURR_CODE) = SRC.DOC_CURR_CODE
                )
                ...
                TGT.X_TRANSFERS = SRC.X_TRANSFERS';
                    EXECUTE IMMEDIATE vvc_sqlstmt;
                  VVC_REC_COUNT := SQL%ROWCOUNT;
                  PROC_INFO_LOG ('PROC_GL_BALANCE_FACT_UPD_PPE',VVC_REC_COUNT||' rows updated',VVC_REC_COUNT||' rows updated');
                ELSE
                                vvc_sqlstmt := '
                                MERGE
                /*+ PARALLEL(TGT 8) */
                INTO W_GL_BALANCE_F PARTITION ('||CURR_MONTH_PARTITION||') TGT USING
                (WITH GLBAL AS
                (SELECT
               ...
               DB_CR_IND,
                ...
                NVL(GLBAL.X_ADDITIONS,0)   + NVL(GLOTHER.X_ADDITIONS,0)   AS X_ADDITIONS,
                NVL(GLBAL.X_RETIREMENTS,0) + NVL(GLOTHER.X_RETIREMENTS,0) AS X_RETIREMENTS,
                NVL(GLBAL.X_TRANSFERS,0)   + NVL(GLOTHER.X_TRANSFERS,0)   AS X_TRANSFERS
                FROM GLBAL,
                GLOTHER
                WHERE 1                                          =1
                AND GLBAL.GL_ACCOUNT_WID                         = GLOTHER.GL_ACCOUNT_WID (+)
                AND GLBAL.DB_CR_IND                              = GLOTHER.DB_CR_IND (+)
                AND REPLACE(GLBAL.ACCT_CURR_CODE, 'NULL', GLBAL.LOC_CURR_CODE) = GLOTHER.DOC_CURR_CODE (+)
                ) SRC ON (TGT.GL_ACCOUNT_WID
                = SRC.GL_ACCOUNT_WID AND TGT.DB_CR_IND = SRC.DB_CR_IND AND TGT.ACCT_CURR_CODE = SRC.ACCT_CURR_CODE)
                WHEN MATCHED THEN
                UPDATE
...
EXCEPTION
WHEN OTHERS THEN
  proc_error_log (0, 'PROC_GL_BALANCE_FACT_UPD_PPE', DBMS_UTILITY.format_error_backtrace () || '~' || vvc_sqlstmt || '~' || vvc_err_msg, SQLERRM );
